...
export const movieZodSchemaQuery = z.object({
    ...
    // Acepta undefined o "" y lo mapea a undefined; si hay valor, debe ser entero positivo
    categoryId: z
        .preprocess(v => {
            if (v === undefined || v === null || v === '') return undefined;
            const n = Number(v);
            return Number.isFinite(n) ? n : v; // deja pasar para que Zod valide el tipo
        }, z.number().int().positive('categoryId debe ser un entero positivo'))
        .optional(),

    // Normaliza + y trim; si queda vacío => undefined
    title: z
        .string()
        .optional()
        .transform(val => {
        ...
        .refine(v => v === undefined || v.length <= 50, {
            message: 'El nombre parcial no puede exceder los 50 caracteres',
        }),
});
